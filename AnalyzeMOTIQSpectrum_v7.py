...
dir_main = 'F:/Data/20210917/XPS_probedet_newFG/mp15_nosignal'
	...
	for file in files: #just iterate through all the files in the directory
		if file.endswith('tdms'):
			...
			if Spectrum == True:
				...
				phase_in_a_cycle+=np.mean(phase.reshape(SegmentsPerFile,numMeasurementsTotal),0)[2*scansize:numMeasurementsTotal]
			OD_file=0
			for k in range(SegmentsPerFile):
				...
				startPulsingProbe1 = 2*scansize+ numMeasurementsTotal*k
				stopPulsingProbe1 = 2*scansize+numMeasurementsProbe1+ numMeasurementsTotal*k
				#phase_slope,phase_offset= fit_to_a_line(ch1_x[startEIT:startEIT+1000],phase[startEIT:startEIT+1000])
				...
				mean_amplitude = np.mean(amplitude[0:2000])-zerovalue
				mean_amplitude2 = np.mean(amplitudePulsingProbe1[3000:4000])-zerovalue
...
average_shots=165
phase_shift=np.mean(np.reshape(phase_shift,(int(numShots/average_shots),average_shots)),1)
...
plt.errorbar(np.arange(numShots/average_shots),phase_shift,yerr=np.sqrt(average_shots)*phase_shift_dir_stem,fmt='ro')
...
stringnamepng = dir_main+"XPS_cyle.png"
