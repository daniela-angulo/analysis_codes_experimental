	...
	for fs in range(0,number_of_files): 	# Iterates through each file in the given directory via expected filename
		...
		for k in range(SegmentsPerFile):
			...
			if filepath.endswith(fileendstring) and k == 0:		# new change
				...
				axes1[0,0].grid()
				axes1[0,0].set_ylim(-1,1.2*np.max(amplitude))
...
axes1[0,2].text(1,  12*phase_shift_dir_std, r'phase shift for CLICK is %1.3f +/- %1.3f (%1.2f) mrad' %(phase_shift_CLICK_dir_averaged, phase_shift_CLICK_dir_stem,phase_shift_CLICK_dir_std), fontsize=10, fontweight='bold' )
axes1[0,2].text(1,  8*phase_shift_dir_std, r'phase shift for NO CLICK is %1.3f +/- %1.3f (%1.2f) mrad' %(phase_shift_NOCLICK_dir_averaged, phase_shift_NOCLICK_dir_stem,phase_shift_NOCLICK_dir_std), fontsize=10, fontweight='bold' )
axes1[0,2].text(1,  16*phase_shift_dir_std, r'phase shift  is %1.3f +/- %1.3f (%1.2f) mrad' %(phase_shift_dir_averaged, phase_shift_dir_stem,phase_shift_dir_std), fontsize=10, fontweight='bold' )
...
axes1[1,2].text(1,  -.3*axes11ylim, r'Phase shifts are %1.3f +/- %1.5f mrad' %(phaseshift_otherway,phase_shift_dir_stem), fontsize=10, fontweight='bold' )
axes1[1,2].text(1,  -.5*axes11ylim, r'Phase shifts are corrected %1.3f +/- %1.3f mrad' %(measured,phase_shift_dir_stem), fontsize=10, fontweight='bold' )
